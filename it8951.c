...
static int wf_a2_match(struct gray4_comparator *comp)
{
	int diff = 0;
	int i;
	for (i = 0; i < 256; i++)
						   ...
						   unsigned int num_clips)
{
	...
	if (!epd->buf)
	{
		...
		memset(epd->buf, 0xff, epd->dev_info.panel_w * epd->dev_info.panel_h / 2); // set pixels to white
		...
		it8951_display_area(epd, 0, 0, epd->dev_info.panel_w, epd->dev_info.panel_h, IT8951_MODE_INIT);
		if (epd->running)
			it8951_standby(epd);
		epd->running = false;
	...
	{
		it8951_memcpy_gray4(tmp_prev_buf, epd->buf, epd->dev_info.panel_w, &clip);
	}
	...
	else
	{
		...
		it8951_memcpy_gray4(tmp_buf, epd->buf, epd->dev_info.panel_w, &clip);
		buf = tmp_buf;
	}
	...
	else
	{
		if (epd->update_mode == -1)
		{
			...
			wf = gray4_auto_wf(tmp_prev_buf, buf, clip_w, clip_h, &comp);
#ifdef WIP_FIX_REFRESH
